...
The text assumes proficiency in general programming concepts, and hence will cut a lot of corners at some expense of readability. Although the narrative is strictly linear and each chapter builds on previous and exercises are not optional, we kept it lightweight enough to be doable in one go. The present document is **not a k reference** and not to be treated as such, the majority of subjects are covered at sufficient depth to support progress but by no means exhaustive.
