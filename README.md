...
**k is different**. You will question many aspects of its design, and it will respond by questioning things that you consider common sense, but this can be a constructive conversation. The first thing newcomers often find puzzling is why the assignment operator is `:` instead of `=`. But give it the benefit of the doubt and try a simple thought experiment. Consider the following code:
