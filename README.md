...
Most programmers and most computer languages will agree that this expression is perfectly valid, but if you show it to a mathematician, be ready to hear "no, it isn't". And once you see what makes him think so, you will see why we assign values with `:` in k. The above expression, although not entirely nonsensical, in k is pretty much always **false**. A different perspective is not necessarily wrong, as there is no tragedy if it someday becomes your own — and that is the right mindset to approach k. 
