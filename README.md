...
**k is different**. At first, you will be questioning its design, and it will respond by questioning things that you consider common sense, but soon enough it will become a constructive conversation. The first thing newcomers frown upon is why the assignment operator is `:` instead of `=`. But give it the benefit of the doubt by trying a simple thought experiment. Consider the following code:

```java
x = x + 1
```

Most programmers and most computer languages will agree that this expression is perfectly valid, but if you show it to a mathematician, be ready to hear "no, it isn't". And once you see what makes him think so, you will see why we assign values with `:` in k. The above expression, although not entirely nonsensical, in k is always **false**. A different perspective is not necessarily wrong, as there is no tragedy if it someday becomes your own — and that is the right mindset to approach k. 
...
The text assumes proficiency in general programming concepts, and hence will cut a lot of corners at some expense of readability. Although the narrative is strictly linear, each chapter builds on all previous and **none of the exercises are optional**, we hope we kept it light and entertaining enough to be digestible in one go.
...
Equipped with everything we have so far, we are going to tackle a little problem. We will look at a k function that actually does something very useful and implements a very familiar algorithm. The subject of game is to identify the algorithm and figure out how it is implementented in k.
